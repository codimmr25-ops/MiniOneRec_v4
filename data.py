...
def combine_sid_tokens(sids):
    """Join all SID levels so variable-length dedup tokens are preserved."""
    return ''.join(sids)
    ...
    def __init__(self, item_file, index_file, tokenizer=None, max_len=2048, sample=-1, test=False, seed=0, category=""):
        ...
        for item_id, sids in self.indices.items():
            if item_id in self.item_feat:
                ...
                # Concatenate all semantic ID levels as the key
                if len(sids) >= 3:
    ...
    def _convert_to_semantic_ids(self, item_ids):
        ...
        for item_id in item_ids:
            ...
            if item_id_str in self.indices:
                ...
                if len(sids) >= 3:
                    # Combine all semantic ID levels
                    combined_sid = combine_sid_tokens(sids)
...
class DPODataset(Dataset):
    """Lightweight JSONL-backed dataset for TRL DPO training."""
                ...
                self.data.append(
                    {
                        ...
                        "chosen": self._normalize_completion(sample["chosen"]),
                        "rejected": self._normalize_completion(sample["rejected"]),
                    }
                )
