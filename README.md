...
| `build_preference_data.py` | Builds preference JSONL from interaction history and item metadata via an external LLM provider         |
| `build_dpo_pairs.py`      | Builds DPO `prompt/chosen/rejected` pairs from preference JSONL and constrained SFT generations         |
| `dpo.sh`                  | Shell script to start the DPO stage                                                                     |
| `dpo.py`                  | Python implementation of the DPO training loop                                                          |
...
### 5. Optional: Preference Data + DPO

This research extension keeps the recommendation prompt unchanged at evaluation time. The generated
`user_preference` text is only used to help construct DPO pairs and is not injected into the final
evaluation prompt.

```bash
python build_preference_data.py --input_csv /path/to/train.csv --item_meta_path /path/to/items.item.json --sid_index_path /path/to/items.index.json --output_jsonl output/research/preferences_train.jsonl --provider minimax --llm_model MiniMax-M2.5 --api_key_env MINIMAX_API_KEY
python build_dpo_pairs.py --model_path output/research/E1_sft/final_checkpoint --preference_jsonl output/research/preferences_train.jsonl --info_file /path/to/info.txt --output_jsonl output/research/E2_pairs_train.jsonl
bash dpo.sh e2
```

### 6. Run the evaluation bash
