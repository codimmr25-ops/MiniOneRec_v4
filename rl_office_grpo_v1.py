...
def train(
    ...
    from minionerec_trainer_office_grpo_v1 import ReReTrainer
    ...
    print(f"only_sid_dataset: {only_sid_dataset}")
    print(f"mask_all_zero: {mask_all_zero}")
    ...
    def parse_report_to(value: str):
        if not value or value.lower() in {"none", "disabled", "false"}:
            return "none"
        targets = [elm.strip() for elm in value.split(",") if elm.strip()]
        if "tensorboard" in targets and importlib.util.find_spec("tensorboard") is None:
            print("TensorBoard is not installed; dropping 'tensorboard' from report_to.")
            targets = [elm for elm in targets if elm != "tensorboard"]
        return targets or "none"

    os.environ['WANDB_PROJECT'] = wandb_project
    os.environ["WANDB_MODE"] = wandb_mode
    logging_dir = logging_dir or os.path.join(output_dir, "runs")
    report_to_targets = parse_report_to(report_to)
    print(f"report_to: {report_to_targets}")
