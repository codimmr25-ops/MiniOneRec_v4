...
except ImportError:  # Optional for provider-only flows
    AutoModel = None
    AutoTokenizer = None
import collections
try:
    import openai
except ImportError:  # Optional when using non-OpenAI providers
    openai = None
import requests
...
def get_openai_batch(model_name, prompt_list, max_tokens, api_info):
    if openai is None:
        raise ImportError("openai package is required when provider='openai'")
    while True:
        try:
