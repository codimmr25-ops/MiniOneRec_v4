...
def _resize_token_embeddings(model, tokenizer) -> None:
...
def load_causal_lm_for_grpo(model_path: str, model_init_kwargs: dict) -> PreTrainedModel:
    ...
    base_model = AutoModelForCausalLM.from_pretrained(peft_config.base_model_name_or_path, **model_init_kwargs)
    _resize_token_embeddings(base_model, tokenizer)
...
class ReReTrainer(Trainer):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            model = load_causal_lm_for_grpo(model, model_init_kwargs)
        else:
        ...
        self.mask_all_zero = mask_all_zero
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        zero_reward_groups = grouped_rewards.abs().sum(dim=1) == 0
        ...
        zero_reward_group_mask = torch.zeros_like(rewards, dtype=torch.bool)
        if self.mask_all_zero:
            zero_reward_group_mask = zero_reward_groups.repeat_interleave(self.num_generations, dim=0)
            advantages = advantages.masked_fill(zero_reward_group_mask, 0.0)
        ...
        zero_reward_group_mask = zero_reward_group_mask[process_slice]
        ...
        self._metrics["zero_reward_group_rate"].append(zero_reward_groups.float().mean().item())
        self._metrics["nonzero_reward_group_rate"].append((~zero_reward_groups).float().mean().item())
        self._metrics["masked_example_rate"].append(zero_reward_group_mask.float().mean().item())
        self._metrics["advantage_abs_mean"].append(advantage_abs_mean.item())
        ...
        return {
            ...
            "zero_reward_group_mask": zero_reward_group_mask,
        }
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        zero_reward_group_mask = inputs.get("zero_reward_group_mask")

        per_token_loss = torch.exp(per_token_logps - per_token_logps.detach()) * advantages.unsqueeze(1)
        per_token_loss = -(per_token_loss - self.beta * per_token_kl)
        if self.mask_all_zero and zero_reward_group_mask is not None:
            active_example_mask = (~zero_reward_group_mask).to(per_token_loss.dtype)
        else:
            active_example_mask = torch.ones_like(advantages, dtype=per_token_loss.dtype)
        self._metrics["active_example_rate"].append(active_example_mask.mean().item())

        if self.dapo:
            active_token_mask = completion_mask * active_example_mask.unsqueeze(1)
            denom = active_token_mask.sum().clamp_min(1.0)
            loss = (per_token_loss * active_token_mask).sum() / denom
        elif self.gspo:
            ...
            per_sequence_loss = -(s_score*advantages - self.beta*sequence_kl)
            denom = active_example_mask.sum().clamp_min(1.0)
            loss = (per_sequence_loss * active_example_mask).sum() / denom
        else:
            per_example_loss = (per_token_loss * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)
            denom = active_example_mask.sum().clamp_min(1.0)
            loss = (per_example_loss * active_example_mask).sum() / denom
        # Log the metrics
